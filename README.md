...
JadwalKRL supports route-specific departures, searches from the current or a selected time, transparent schedule-snapshot freshness, and device-local saved routes with their next scheduled train.
...
docker build -t jadwalkrl . && docker run -p 8080:8080 jadwalkrl
```
...
Saved routes remain private to the current browser through `localStorage`. The Saved page requests the current next scheduled train from JadwalKRL when opened, focused, manually refreshed, or when its countdown ends.
...
This downloads CSV files into the ignored `.cache/` directory and replaces the local SQLite data. The importer preserves each train's ordered stops based on its departure times. The source does not provide per-stop arrival times, so JadwalKRL uses the departure time for both arrival and departure until a more detailed source is available.
...
Some API routes terminate beyond the station-picker catalog. JadwalKRL records those published terminal destinations with deterministic `KCI_DEST_*` codes so the journey remains searchable; they are marked as destination-only stations.
...
```sh
turso db create jadwalkrl
export TURSO_DATABASE_URL="$(turso db show --url jadwalkrl)"
export TURSO_AUTH_TOKEN="$(turso db tokens create jadwalkrl)"
task migrate:turso
```

`task migrate:turso` applies this repository's migrations to Turso first, then copies all JadwalKRL tables. It refuses to overwrite a non-empty Turso database unless run with `go run ./cmd/migrate-turso --replace`. Deployments only need the two `TURSO_*` variables; do not set `DATABASE_URL` to the Turso URL.
...
JadwalKRL is an independent portfolio project and is not affiliated with KAI or KAI Commuter.
